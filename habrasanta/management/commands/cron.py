    ...
    def match_season(self, *args: Any, **options: Any) -> None:
        ...
        with transaction.atomic():
            ...
            for cluster in clusters:
                ...
                if len(cluster):
                    self.stdout.write(
                        "Gonna match cluster '{}'...".format(",".join(cluster))
                    )
                ...
                if len(participants) < 3:
                    if len(participants) > 0:
                        self.stdout.write(
                            self.style.ERROR(
                                "Not enough participants to match cluster {} in season {}!".format(
                                    ",".join(cluster), season.id
                                )
                            )
                        )
    ...
    def send_chat_notifications(self, *args: Any, **options: Any) -> None:
        ...
        with transaction.atomic():
            ...
            for result in queryset:
                ...
                User.objects.filter(pk=result["to_user"]).update(
                    last_chat_notification=now
                )
                self.stdout.write(
                    self.style.SUCCESS("User {} notified".format(result["to_user"]))
                )
            else:
